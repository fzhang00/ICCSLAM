\chapter{Literatures Review}\label{ch:Review}
...
% word checked
Camera model relates measurements on image plane to measurements of
...
\begin{itemize}
  \item $\mathbf{x_k}$: a vector that describes the vehicle position and
orientation.
  \item $\mathbf{u_k}$: the control vector applied at time $k-1$ to drive the
vehicle to $\mathbf{x_k}$ at time step k
  \item $\mathbf{p_i}$: a vector that describes the location of the $i^{th}$
landmark. All landmarks locations are assumed to be time invariant.
  \item $\mathbf{z_{i,k}}$: observation of the location of the $i^{th}$
landmark taken from the vehicle's location at time step $k$
\end{itemize}

\noindent A complete state vector contains 

$$\begin{bmatrix}\hat{\mathbf{x}}_k \\ \hat{\mathbf{p}}_k \end{bmatrix}$$ 

\noindent and the state covariance matrix contains

$$\begin{bmatrix}
\mathbf{P_{xx}} & \mathbf{P_{xp}} \\
\mathbf{P_{px}} & \mathbf{P_{pp}} 
\end{bmatrix} $$
...
the prediction (i.e. only $\mathbf{x_k}$ and $\mathbf{P_{xx}}$ are
...
a SLAM problem. The errors in estimates of landmarks becomes more and
more correlated as the vehicle ventures through the unknown
environment. The error eventually become fully correlated which means
...
% word checked
Many researchers reported filter divergence due to
...
optimistic estimation occurs and the filter can no longer 
update the estimates effectively.

Huang investigated on properties and consistency of nonlinear
two-dimensional EKF SLAM problem, derived and proved a number of
