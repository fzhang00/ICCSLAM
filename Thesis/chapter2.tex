...
There are two popular camera calibration methods. Tsai \cite{tsai}
requires at least 5 points in 3D space with known coordinate, and their
corresponse coordinate in the image plane. With Tsai's method, only
one snapshot of the calibration target is needed. The disavantage of
this method is that the 3D position of the points is difficult to
obtained. Zhang \cite{zhang} proposed a more flexible method that
...
reliable result, 10 view fo the checkboard with different translation
...
implemented in OpenCV, and is used to calibrate the camera for this
work. 
...
some weighting coeffi cient), and then comparing this difference to a
...
method was not only suffi cient but in many cases gave more
