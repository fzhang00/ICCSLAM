...
drift. In figure \ref{fltfig:2} the deletion and addition of features
is marked by a vertical gray dash line. Parameters $y_i$ and $z_i$ are
most affected. 

The drift in deleted features parameters are due to the
error in SUAS localization estimates. In order to compensate the error
in localization (Figure \ref{fltfig:4}), the algorithm made adjustment
on the estimate of features parameters so that the combined
result agrees with the measurement. Since all features are transformed
into world frame using the estimated SUAS localization result, the
combined final result shows a steady pattern. The deleted features no
longer had their parameters updated every iteration. Therefore, after
these value are transformed into world frame, they carries the error
in SUAS localization estimates starting from their deletion.
